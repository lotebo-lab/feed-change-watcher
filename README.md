...
| question | page |
|---|---|
| How do I get only new items from an RSS feed? | https://apify.com/lotebo-lab/feed-change-watcher/examples/get-only-new-items-from-an-rss-feed |
| What is new across several release feeds today? | https://apify.com/lotebo-lab/feed-change-watcher/examples/watch-several-release-feeds-in-one-run |
| How do I watch release feeds and get only new versions? | https://apify.com/lotebo-lab/feed-change-watcher/examples/watch-release-feeds-for-new-versions |
| How do I track new security advisories across vendor feeds? | https://apify.com/lotebo-lab/feed-change-watcher/examples/watch-vendor-security-advisory-feeds-for-new-entries |
| How do I see only new tender notices from many agency feeds? | https://apify.com/lotebo-lab/feed-change-watcher/examples/read-many-tender-feeds-and-see-only-new-notices |
| How do I monitor regulator news feeds without rereading old items? | https://apify.com/lotebo-lab/feed-change-watcher/examples/monitor-regulator-news-feeds-without-rereading-old-items |
| How do I send only new feed items to n8n or Make, never duplicates? | https://apify.com/lotebo-lab/feed-change-watcher/examples/send-only-new-feed-items-to-n8n-or-make |
| Which of my feeds stopped working this run? | https://apify.com/lotebo-lab/feed-change-watcher/examples/check-which-of-my-feeds-stopped-working |

Who runs it: engineering and integration teams watching vendor changelogs and release feeds, to catch the version that breaks an integration; operations and support, keeping a record of incidents on the status feeds of the services they depend on; procurement, legal and policy people following supplier blogs, standards bodies and regulators; and anyone who has to be able to say what changed last week.

## Run it on a schedule (this is the point)

What this Actor sells is the difference between two runs, so one run on its own is half of it. **The first run on a feed returns the feed as it stands today**: every item it carries at that moment, marked `first_run: true` in the `REPORT`. It is the run after that which does the job you came for.
...
| `maxSources` | integer | no | 20 | 1 to 500 feeds read in this run; feeds beyond the number are left for the next run |
...
An example input, using the value the schema prefills for `feedUrls`:
...
The list of item ids already returned is kept per feed URL in a named key-value store that belongs to your account. Up to 5000 ids are kept per feed, oldest dropped first (`MAX_SEEN_IDS_PER_SOURCE` in `src/state.py`).

## What comes out

The dataset has two kinds of row, told apart by `rowType`: `feed-item` for a new item, and `summary` for the single run summary row that every successful run writes, new items or none. Filter on `rowType` to keep only the items. These are the fields declared in [`.actor/dataset_schema.json`](.actor/dataset_schema.json) and checked against what the code writes by `tests/test_schemas.py`.

An item row, besides `rowType`, has exactly these seven fields and nothing else:
...
The dataset ships two ready-made views, declared in the same schema: **New items** and **By source**. Every run also writes a `REPORT` record in the key-value store with the same counts plus `robotsFetches` and a `perSource` list (`source_url`, `source_title`, `status`, `error`, `items_in_feed`, `new_items`, `first_run`). `items_in_feed` next to `new_items` is the part that matters when the answer is "nothing new": it shows the feed was really read and was really quiet.
...
The row below comes from an end-to-end run of `src/main.py` recorded on 2026-09-20: the second run of a pair, with the feeds served by a **local test server on `127.0.0.1`** from `tests/fixtures/run2`, not by a publisher. Between the two runs one post was added to the blog feed and nothing changed in the status feed. The blog feed carried four items at that moment, and this is the only row the run returned:
...
The first run of the same pair returned five rows: everything the two feeds carried. That is the baseline run. Both runs were recorded before the `rowType` field and the summary row were added, so neither appears in the snippets above. The log files are kept in our build workspace and are **not** published here: `logs/` is in `.gitignore`.
...
- **A feed is fetched only when you list it.** This Actor makes one HTTP GET per feed URL in your input, follows redirects, reads at most 10 MB (`MAX_RESPONSE_BYTES` in `src/main.py`), and never crawls, follows links or discovers feeds on its own.
...
- **`robots.txt` is not the whole answer.** A feed can be open in `robots.txt` and closed to automated clients by the site's terms or by your agreement with it.
...
**The offline test suite.** Four files, run with the plain interpreter and no network, from the root of this repository on 2026-09-23:
...
What they cover: `test_feeds.py` parses RSS 2.0, RSS 1.0 and Atom documents and checks the seven-field allow list, the date normalisation and the "which of these are new?" decision; `test_robots.py` drives the `robots.txt` gate with an injected fetcher, including a malformed file, a 404 and a connection error, and checks that none of them blocks a feed; `test_schemas.py` compares `.actor/input_schema.json`, `.actor/dataset_schema.json` and `.actor/actor.json` against what the code actually does, field by field and default by default; `test_charging_e2e.py` runs the whole flow twice over a fixture and checks the bill of each run.

**Against real feeds, on the Apify platform.** These are runs on Apify against public feeds, not local fixtures:

- run `Y8drvcQt2D4h6a5EF`, the second run in a row on `https://news.ycombinator.com/rss`: `30 item(s) in the feed, 0 new`, and one summary row saying `No new items: 1 of 1 feed(s) were read and every item they list had already been reported by an earlier run`. That is the whole point of the Actor, measured;
- run `EsSLbti3Ge74zFGNH` on the same feed found 10 new items and wrote 11 rows;
- runs `OljBI69qNps2VoaRX` and `nXD1GPHaPMMmgWqDq` both returned `newItems: 0` with `chargedEvents: 1`, which is the proof that a quiet run pays for the feed it read and **not** for the change report;
...
- **No paid bill has ever come out of this Actor.** Charging has been exercised on the platform and the event counts above were read back from the run's own dataset, but no invoice has ever been produced by it. A charge that fails or times out is a warning in the log, never the end of the run.
- **No publisher has ever refused us in `robots.txt`.** The gate is covered offline, with an injected fetcher, so `skipped_by_robots` is proven in test, not in the wild.
- **No long watch.** The longest sequence we measured on one feed is a handful of consecutive runs inside a single day, so the 5000-id ceiling per feed has never been reached by a real run.
- **Not every feed dialect in the wild.** The parser was exercised against the documents in `tests/` and the public feeds above. A feed it cannot read is reported as `failed`, with the reason, and never silently dropped, but we cannot claim it parses everything that calls itself a feed.
- **No `Content-Type` check**, as stated above: the decision is made by the XML parser, not by the header the server declares.
...
The Actor is written in Python. Parsing uses the standard library (`xml.etree`), not `feedparser`, so there is no extra dependency to break, and a failing feed never stops the run. This Actor was built with the help of AI, and every module has an offline automated test in `tests/`.
